...
function stBlock(b){
  ...
  $('#st-block-body').innerHTML = `<h2>${esc(b)}</h2>
    <p class="muted small">Cada nivel tiene ${PACK} ejemplares parecidos entre sí (por familia; primero prioridad A y los que han salido en visu). Al pulsar <b>Estudiar</b> primero los ves con su nombre y luego los escribes tú, como en el examen. Con 3 estrellas (90 % bien) el nivel queda superado.</p>
    ...
    <div class="niveles">${P.map((p, i) => {
      ...
      return `<div class="nivel${s.wstars === 3 ? ' done' : ''}"><div><b>Nivel ${i + 1}</b> ${starsHTML(s.wstars || 0)} ${s.wbest != null ? `<span class="small muted">mejor: ${s.wbest}/${p.length}</span>` : ''}
        ...
        <div class="nv-btn"><button class="link" data-a="write" data-i="${i}">solo escribir</button><button class="primary" data-a="learn" data-i="${i}">${s.wstars ? 'Repetir' : 'Estudiar'}</button></div></div>`;
    }).join('')}</div>`;
...
function stLearn(p, i){
  playShell(`${CUR.b} · ${i == null ? 'Repaso' : 'Nivel ' + (i + 1)} · Aprender`);
  ...
  const show = () => {
    ...
    $('#st-play-body').innerHTML = `<div class="learn">
      ...
      <div class="learn-side">
        <div class="learn-count">${k + 1} de ${p.length}</div>
        ...
        <div class="learn-nav"><button id="st-prev" ${k ? '' : 'disabled'}>←</button>
          ${k < p.length - 1 ? '<button class="primary" id="st-next">Siguiente →</button>' : '<button class="primary" id="st-go">Ahora, a escribirlos →</button>'}</div>
    ...
    if ($('#st-go')) $('#st-go').onclick = () => stWrite(p, i);
    CUR.keys = ev => {
