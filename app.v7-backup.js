...
function stBlock(b){
  ...
  $('#st-block-body').innerHTML = `<h2>${esc(b)}</h2>
    <p class="muted small">Cada nivel tiene ${PACK} ejemplares parecidos entre sí (por familia; primero prioridad A y los que han salido en visu). Sigue el orden: <b>1 · Aprender</b> (tarjetas), <b>2 · Elegir</b> (4 opciones) y <b>3 · Escribir</b> (como en el examen: escribes el nombre científico). El nivel está superado con 3 estrellas en los dos juegos.</p>
    <div class="row"><button id="st-mix">Mezcla: elegir (20)</button><button class="primary" id="st-mixw">Mezcla: escribir (20)</button>${fallos.length ? `<button id="st-fail">Repasar mis fallos (${fallos.length})</button>` : ''}</div>
    <div class="niveles">${P.map((p, i) => {
      const s = SS[pkey(b, i)] || {}, fam = [...new Set(p.map(e => e.familia).filter(Boolean))].slice(0, 4).join(', ');
      return `<div class="nivel${s.stars === 3 && s.wstars === 3 ? ' done' : ''}"><div><b>Nivel ${i + 1}</b> <span class="small muted">elegir</span> ${starsHTML(s.stars || 0)} <span class="small muted">escribir</span> ${starsHTML(s.wstars || 0)}
        <div class="small muted">${esc(fam || p.map(e => e.sci).slice(0, 3).join(', '))}${p.some(e => e.visu) ? ' · <span class="badge v">visu</span>' : ''}</div></div>
        <div class="nv-btn"><button data-a="learn" data-i="${i}">1 · Aprender</button><button data-a="play" data-i="${i}">2 · Elegir</button><button class="primary" data-a="write" data-i="${i}">3 · Escribir</button></div></div>`;
    }).join('')}</div>`;
  ...
  $('#st-mixw').onclick = () => stWrite(shuffle(L.slice()).slice(0, 20), null);
  if (fallos.length) $('#st-fail').onclick = () => stWrite(shuffle(fallos.slice()).slice(0, 20), null);
}
...
function stLearn(p, i){
  ...
  const show = () => {
    ...
    if ($('#st-go')) $('#st-go').onclick = () => i == null ? stWrite(p, i) : stGame(p, i);
  };
  show();
...
function stGame(p, i){
  ...
  const fin = () => {
    ...
    $('#st-play-body').innerHTML = `<h2>${ok} / ${items.length} ${starsHTML(stars)}</h2><p class="score">${pts} puntos</p>
      ...
      ${i != null ? '<button class="primary" id="st-towrite">Ahora escríbelos →</button>' : ''}<button id="st-toblock">Volver al bloque</button></div>`;
  ...
  CUR.keys = ev => { if (ev.target.tagName === 'INPUT') return; const n = +ev.key; if (n >= 1 && n <= 4) { const b = $('#st-play-body').querySelectorAll('.opt')[n - 1]; if (b) b.click(); } };
  show();
}
...
function stWrite(p, i){
  playShell(`${CUR.b} · ${i == null ? 'Mezcla' : 'Nivel ' + (i + 1)} · Escribir`);
  const items = shuffle(p.slice()); let k = 0, ok = 0, pts = 0, racha = 0; const fallos = [];
  const show = () => {
    ...
    $('#st-play-body').innerHTML = `<div class="st-count">${k + 1} / ${items.length} <span class="st-pts">${pts} pts${racha > 1 ? ` · racha ×${racha}` : ''}</span></div>
      ...
      <div class="st-write"><input id="st-in" type="text" autocomplete="off" autocapitalize="off" spellcheck="false" placeholder="${isBio(e) ? 'Nombre científico (género y especie)…' : 'Nombre…'}">
        <button class="primary" id="st-send">Comprobar</button><button id="st-hbtn">Pista</button></div>
  ...
  const fin = () => {
    ...
    $('#st-play-body').innerHTML = `<h2>${ok} / ${items.length} ${starsHTML(stars)}</h2><p class="score">${pts} puntos</p>
      ...
      <div class="row"><button id="st-again">Repetir</button>${fallos.length ? '<button id="st-refail">Escribir solo los fallos</button>' : ''}
    ...
    if ($('#st-refail')) $('#st-refail').onclick = () => stWrite(fallos, null);
